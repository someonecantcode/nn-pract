% Building
With these operators, we can define any function atomically and build up to
a multilayer perceptron. A neural network is composed of layers which are composed of neurons. 
This structure (with a non-linear function) allows the approximation of any shape it or desired behavior. 
...
\begin{definition}[Neuron]
    ...
    \begin{align*}
        ...
        \begin{bmatrix}
            b
        \end{bmatrix}, \\ 
        & \quad i \text{ being the number of inputs.}
    \end{align*}
\end{definition}
...
\begin{definition}[MLP]
    ...
    \begin{align*}
        ...
        \begin{bmatrix}
          ...
          \end{bmatrix}
        \end{bmatrix}, \\
        & \quad a,b,c \text{ being the size of each layer.}
    \end{align*}
\end{definition}
...
After forward passing inputs, we will receive an output matrix. With the output, we will calculate the loss and
run back propagation for the individual gradients and apply gradient descent on the parameters to reduce the loss value.
In turn, this will result in a model that is better suited toward a desired result. Repeating this process will
eventually lead to any function reaching a minimum loss.
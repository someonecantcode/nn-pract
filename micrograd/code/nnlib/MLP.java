...
class Neuron extends Module {
    ...
    public Value call(Value[] input) {
        ...
        act.label = "act_final";
        return this.nonlinear ? act.GELU() : act; // RELU = 0 causes all gradients to be 0.
    }
...
public class MLP extends Module {
    ...
    public MLP(int numberinputs, int[] numberoutputs) { 
        ...
        for(int i = 0; i < numberoutputs.length; i++) { 
            this.layers[i] =  new Layer(chaining[i], chaining[i+1], (i != numberoutputs.length - 1)); //each inbetween layer is activation RELU
        }
    }
